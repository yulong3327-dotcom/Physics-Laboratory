...
import { OpticalSymbol } from '../optics/OpticalSymbol'
  ...
  return <div className="app-shell optics-lab">
    ...
    <div className={`optics-workspace ${panel ? `optics-show-${panel}` : ''}`}>
      ...
      <aside className="optics-properties" aria-label="光学属性面板"><div className="section-heading"><span>{selected ? '组件属性' : '实验概览'}</span><IconButton label="关闭光学属性面板" className="optics-properties-toggle" onClick={() => setPanel(null)}><X size={16} /></IconButton></div><div className="property-content">{selected ? <OpticsProperties key={selected.id} component={selected} /> : <><div className="property-title">光的传播与成像<span className="subtle-badge">Geometrical optics</span></div><p className="optics-description">拖动元件搭建光路，逐片调节透镜焦距与间距，观察多次成像。显微镜、望远镜示例由独立凸透镜组合而成。</p><div className="optics-component-list">{store.scene.components.map(c => <button key={c.id} onClick={() => store.select(c.id)}><span>{c.label || opticalLibrary[c.kind].name}</span><span>{opticalLibrary[c.kind].name}</span></button>)}</div>{!store.scene.components.length && <p className="empty-property">从元件库放置组件，或载入一个实验示例。</p>}</>}
        ...
        <p className="optics-model-note">单片及组合透镜采用理想薄透镜近轴模型；玻璃板、三棱镜按折射定律追迹。颜色表示光源波长，暂不模拟色散、衍射和干涉。</p>
      </div></aside>
    </div>
...
function OpticsProperties({ component: c }: { component: OpticalComponent }) {
  ...
  return <>
    <div className="property-title optics-property-title"><svg viewBox="-27 -24 54 48" width="54" height="48" aria-hidden="true"><OpticalSymbol component={c} thumbnail /></svg><span>{opticalLibrary[c.kind].name}<small>{opticalLibrary[c.kind].nameEn}</small></span></div>
    ...
    {c.kind !== 'point-source' && c.kind !== 'laser' && <NumberControl label={c.kind === 'object' ? '物高' : c.kind === 'parallel-source' ? '光束宽度' : lens ? '透镜口径' : '组件高度'} value={c.height} min={0.2} max={200} step={0.5} unit="cm" slider onChange={v => update({ height: v, ...(c.kind === 'aperture' ? { opening: Math.min(c.opening, v) } : {}) })} />}
    ...
    <div className="optics-component-actions"><button className="text-button" onClick={() => update({ angle: c.angle + 15 })}><RotateCw size={14} />旋转 15°</button><button className="text-button" onClick={() => store.duplicate(c.id)}><Copy size={14} />复制</button><IconButton label="删除光学组件" danger onClick={() => store.remove(c.id)}><Trash2 size={16} /></IconButton></div>
  </>
}
...
function ImageReadout() {
  ...
  return <section className="optics-measurements"><div className="optics-field-heading">成像分析<span>单透镜 / 多透镜</span></div>{!images.length ? <p className="optics-field-note">放置同轴的物体与透镜，查看物距、像距和放大率。多片透镜会显示中间像与最终像。</p> : images.map(m => <div key={`${m.lensId}-${m.objectId}`} className="optics-measurement-card" data-testid="optics-image-measurement"><div className="optics-image-nature"><span className="status-dot" /><b>{m.caption || (m.nature === 'infinity' ? '不成有限远像' : `${m.magnification! < 0 ? '倒立' : '正立'}${Math.abs(m.magnification!) > 1.005 ? '放大' : Math.abs(m.magnification!) < 0.995 ? '缩小' : '等大'}${m.nature === 'real' ? '实像' : '虚像'}`)}</b></div><dl><div><dt>物距 u</dt><dd>{format(m.objectDistance)} <small>cm</small></dd></div><div><dt>像距 v</dt><dd>{format(m.imageDistance)} <small>cm</small></dd></div><div><dt>{(m.lensIds?.length || 1) > 1 ? '累计放大率 m' : '放大率 m'}</dt><dd>{format(m.magnification)}</dd></div></dl><div className="optics-formula">1/f = 1/u + 1/v<span>{(m.lensIds?.length || 1) > 1 ? 'm = m₁ × m₂ × …' : 'm = −v/u'}</span></div></div>)}{!running && <p className="optics-field-note">光线已暂停，参数分析仍实时更新。</p>}</section>
}
