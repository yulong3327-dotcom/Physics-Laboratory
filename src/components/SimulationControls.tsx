...
export const simulationStatusNames = { no_source: '未接入电源', open: '开路', operating: '通电', short_circuit: '短路', error: '无法求解' }
...
export function SimulationControls() {
  ...
  return <div className={`simulation-controls ${result?.status || ''}`}>
    ...
    {result && <><Activity size={14} /><span role="status" data-testid="simulation-status">{simulationStatusNames[result.status]}</span><output>{formatReading(result.totalCurrent, 'A')}</output></>}
...
export function ComponentExperiment({ component }: { component: CircuitComponent }) {
  ...
  return <section className="experiment-properties" aria-label="实验参数">
    ...
    {component.type === 'lamp' && <p className="experiment-note">按额定参数计算稳态电阻；亮度随功率变化。</p>}
    ...
    {state && !meter && <dl className="measurement-values"><div><dt>电流</dt><dd>{formatReading(state.current, 'A')}</dd></div><div><dt>端电压</dt><dd>{formatReading(state.voltage, 'V')}</dd></div><div><dt>功率</dt><dd>{formatReading(state.power, 'W')}</dd></div></dl>}
  </section>
}
